...
export default class Clean extends React.Component {
    ...
    async typeCleanCheck(UID, postID, page) {
        switch (page) {
            case "Overview":
                this.demoCleanButton();
                break;
            case "Find Job":
                await this.CleanNow(UID, postID);
            default:
                break;
    ...
    async CleanNow(UID, postID) {
        try {
            ...
            setTimeout(async () => {
                ...
                const user = "" + UID;
                const post = "" + postID;
                await this.storage.StoreData("cleaning", user, post);
            }, 2000)
        } catch (e) {
    ...
    render() {
        return (
            <Library.Base.Card key={this.state.value.postID} style={{ borderRadius: 10, elevation: 0 }}>
                <Library.Base.CardItem
                            ...
                            <Library.Box.Row style={{ margin: 5 }}>
                                <Library.Box.Col size={20} style={{ justifyContent: "center", alignItems: "center", margin: 5 }}>
                                    <Library.Native.Image
                                        source={/^https?:\/\//.test(this.state.value.image) ? { uri: this.state.value.image } : this.state.value.image}
                                ...
                                <Library.Box.Col size={60} style={{ justifyContent: "center", flexDirection: "row-reverse", margin: 5 }}>
                                    ...
                                    <Library.Box.Row style={{ alignItems: "center" }}>
                                        <Library.Base.Text>
                                            {`By ${this.state.value.author.full}`}
                                        </Library.Base.Text>
                                    </Library.Box.Row>
                                </Library.Box.Col>
                            </Library.Box.Row>
                                ...
                                this.state.expand.length === 0 ? <Library.Base.Text></Library.Base.Text> : (
                                    <>
                                        ...
                                        <Library.Box.Row key={2} style={{ backgroundColor: "#e4f7fd" }}>
                                            ...
                                            <Library.Box.Col size={80} style={{ justifyContent: "center" }}>
                                                <Library.Base.Text>{this.state.expand[0].hour} Hour Clean</Library.Base.Text>
                                            </Library.Box.Col>
                                        </Library.Box.Row>
                                        ...
                                        <Library.Native.TouchableOpacity
                                            key={4}
                                            onPress={() => {
                                                console.log("Task: ", this.state.expand[0].task);
                                                return this.dialog.topicDescriptionAndTaskDialogMethod(this.state.expand[0])
                                            }}>
                    ...
                    <Library.Box.Grid style={{ margin: 10 }}>
                        <Library.Box.Col
                            ...
                            <Library.Native.TouchableOpacity onPress={() => this.typeCleanCheck(this.state.value.id, this.state.value.postID, this.state.page)}>
                                <Library.Animated
